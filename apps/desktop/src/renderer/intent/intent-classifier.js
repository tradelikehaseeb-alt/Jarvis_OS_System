...
const INTENT_RULES = [
    ...
    },
    {
        ...
        test: (text, tokens) => includesAnyPhrase(text, [
            "automate",
            "automation",
            "run script",
            "workflow",
    ...
    },
    {
        ...
        test: (text, tokens) => matchesAnyPattern(text, [
            /\bsearch\b/,
            /\blook\s+up\b/,
            /\blookup\b/,
            /\bfind\s+me\b/,
    ...
    },
    {
        ...
        test: (text, tokens) => text.endsWith("?") &&
            text.length < 60 &&
            tokens.length <= 8 &&
            !includesAnyToken(tokens, [
                "plan",
...
export function loadingMessageForIntent(intent) {
    if (intent === "plan") {
